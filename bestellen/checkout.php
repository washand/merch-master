...
<body>
  ...
  <script>
    ...
    function updateSummary() {
      ...
      // Build items HTML with detailed specs
      let itemsHtml = '';
      CART.forEach((item, idx) => {
        const productName = item.product_naam || (item.mdl ? `${item.mdl.brand} ${item.mdl.name}` : item.sku);
        const quantidade = item.aantal || item.qty || 1;

        // Build position labels
        const posLabels = {'voorkant': 'Voorkant', 'achterkant': 'Achterkant', 'linkerborst': 'Linkerborst', 'rechterborst': 'Rechterborst'};
        const posLabel = (item.posities || []).map(p => posLabels[p.positie] || p.positie).join(' + ') || 'Voorkant';

        // Build techniek per positie
        const techMap = {'dtf': 'DTF', 'zeefdruk': 'Zeefdruk', 'zeef': 'Zeefdruk', 'borduren': 'Borduren'};
        let techDetails = '';
        if (item.technieken && item.technieken.length > 0) {
          const techByPos = {};
          item.technieken.forEach(t => {
            const pos = t.positie || 'voorkant';
            const tech = techMap[t.techniek] || t.techniek;
            techByPos[pos] = tech;
          });
          const techLines = Object.entries(techByPos).map(([pos, tech]) => `${posLabels[pos] || pos}: ${tech}`);
          techDetails = techLines.join('<br>');
        } else if (item.techniek) {
          techDetails = techMap[item.techniek] || item.techniek;
        }

        // Build maten display
        let matenLabel = '';
        if (item.maten && Object.keys(item.maten).length > 0) {
          const matenMap = {'xs': 'XS', 's': 'S', 'm': 'M', 'l': 'L', 'xl': 'XL', 'xxl': 'XXL', 'xxxl': '3XL', '0': 'XS', '1': 'S', '2': 'M', '3': 'L', '4': 'XL', '5': 'XXL', '6': '3XL'};
          const matenArray = [];
          Object.entries(item.maten).forEach(([size, qty]) => {
            if (qty > 0) {
              const sizeName = matenMap[size] || size;
              matenArray.push(`${qty}× ${sizeName}`);
            }
          });
          if (matenArray.length > 0) matenLabel = matenArray.join(', ');
        }

        itemsHtml += `<div class="summary-item">
          <div class="summary-row">
            <span class="label"><strong>${quantidade}× ${productName}</strong></span>
          </div>
          <div class="summary-details" style="font-size:0.85rem;color:#666;margin:0.3rem 0;line-height:1.5;">
            <div><strong>SKU:</strong> ${item.sku}</div>
            <div><strong>Kleur:</strong> ${item.kleur_naam || '–'}</div>
            ${matenLabel ? `<div><strong>Maten:</strong> ${matenLabel}</div>` : ''}
            <div><strong>Positie:</strong> ${posLabel}</div>
            <div><strong>Techniek:</strong><br>${techDetails}</div>
          </div>
        </div>`;
      });
